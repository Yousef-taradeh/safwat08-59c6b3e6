 ...
 * ======================================
 *
 * CRITICAL ARCHITECTURE — why every decision below matters for egress:
 *
 * 1. `key` on <video>: MUST be stable per content item.
 ...
 *    FIX: use a stable `videoMountKey` ref that only increments on REAL item changes.
 *
 ...
 *    FIX: use resolvedSrc only; initialize it to the remote URL synchronously.
 *
 * 3. `preload` attribute MUST be explicitly set to 'none' on the video element.
 *    Samsung Tizen / LG WebOS browsers default to 'auto' and will prefetch range
 *    chunks even from blob: URLs if preload is unset.
 *    FIX: set preload="none", then load() only after src is committed.
 *
 * 4. Watchdog MUST NEVER call video.load() or reassign video.src.
 *    Even blob: src reassignment causes the browser to re-open the internal blob stream.
 *    FIX: recovery is video.play() only.
 *
 * 5. Realtime subscription MUST use a ref for playlist.id to avoid re-creating channels.
 *    Re-creating the channel triggers quickRefresh which updates the content array ref
 *    even when content is identical, cascading into effects.
 */
...
export function ContentRenderer({
  ...
  // ── videoMountKey: stable key for the <video> element.
  // ONLY incremented when the content item actually changes (by ID).
  // This prevents React from re-mounting the video element on quickRefresh
  // content array reference changes, which would trigger a new 206 request.
  ...
  useEffect(() => {
    ...
    // ── Update stable mount key ONLY when the actual video item (by ID) changes.
    // This prevents video element re-mount on every quickRefresh content array update.
    ...
    // This was causing a new 206 stream every time the content array got a new reference.
  ...
  // Even reassigning a blob: src causes the browser to re-open the internal blob
  // stream and can trigger the CDN to serve a new 206 range request.
  // Recovery is video.play() ONLY.
  useEffect(() => {
    const video = videoRef.current;
    ...
    const id = setInterval(() => {
      ...
      // Recovery: call play() ONLY — NEVER reassign video.src or call video.load()
      // Reassigning src (even to a blob:) causes a new range request in some browsers.
      console.log('[Watchdog] 🔄 Attempting recovery via play() only — no src/load touch');
    ...
    return (
      <div className="absolute inset-0 overflow-hidden bg-black">
        {fb.type === 'image'
          ...
          : <video src={fb.url} className="w-full h-full object-cover" autoPlay muted loop playsInline preload="none"
    ...
    <div className="absolute inset-0 overflow-hidden bg-black">
      ...
      <div className="absolute inset-0" style={transitionStyles.current}>
        {currentContent.type === 'image' ? (
          ...
          <video
            // CRITICAL: videoMountKey is stable per content item ID.
            // It does NOT change when the content array gets a new reference (quickRefresh).
            // Using currentContent.id directly as key was causing re-mounts on every refresh.
            key={videoMountKey}
            ref={videoRef}
            src={resolvedSrc}
            className={cn('w-full h-full', scalingClass)}
            autoPlay
            muted
            // preload="none" prevents Samsung Tizen / LG WebOS from prefetching range chunks.
            // The video will load via autoPlay after the element mounts with the src.
            preload="none"
